...
export function searchPosition(game, searchDepth, hashTable, masterAncient, evalCap, maxPly) {
	...
	function orderMoves(moves, hash, game) {
		// Get necessary information from the game state
		...
		// Loop through all the moves and compute their bonuses
		moves.forEach(move => {
			// Recompute hash and initialize bonus
			const newhash = recomputeZobristHash(hash, board, move, turn);
			let bonus = 0;

			// Compute bonus based on move priority
			...
			// Determine the type of piece being moved and adjust bonus accordingly
			...
			// Check for additional bonuses for certain move types
			if (move.search(/\+/) !== -1)
				bonus += 10;
			if (isCapture(move)) {
		...
		// Sort the moves based on their bonuses
		orderedMoves.sort((a, b) => b.bonus - a.bonus);

		// Refine the ordered moves by only keeping the move strings
