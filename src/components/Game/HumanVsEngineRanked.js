...
import SettingsModalRanked from './Modals/SettingsModalRanked';
import GameoverModalRanked from './Modals/GameoverModalRanked';
...
function HumanVsEngineRanked() {
    ...
    const [openGameoverModalRanked, setOpenGameoverModalRanked] = useState(false);
    ...
    useEffect(() => {
        ...
        if (gameOver) {
            ...
            setOpenGameoverModalRanked(true);
            return;
        }
    ...
    function undoGame() {
        if (game.current.turn() !== playerColor) return; // Disables the undo button whilst the AI is thinking
        setWaitingForHint(false); //Disables hint
        game.current.undo();
        ...
        setHistory(history => {
            ...
            newHistory.pop();
            if (game.current.turn() !== playerColor) { // Undo AI move
                game.current.undo();
                chessEngineWorker.current.postMessage({ type: 'undo' });
                newHistory.pop();
            }
    ...
    function newGame() {
        ...
        setHistory([]);
        setGameoverStatus(0);
        setOpenGameoverModalRanked(false);
        setWaitingForHint(false);
    ...
    return (
        <div className="game">
            ...
            {
                openGameoverModalRanked &&
                <GameoverModalRanked
                    playerColor={playerColor}
                    statusCode={gameoverStatus}
                    startNewGame={newGame}
                    closeModal={() => setOpenGameoverModalRanked(false)}
                />
            }
            {
                getPosition &&
            ...
            <div className="bounding-box">
                ...
                <PlayerInfo name="You" isThinking={(gameoverStatus === 0) && (game.current.turn() !== playerColor)} thinkingText={`You is Thinking...`} />
            </div>
...
export default HumanVsEngineRanked;